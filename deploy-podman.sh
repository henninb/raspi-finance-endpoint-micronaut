...
# Podman deployment script for raspi-finance-endpoint-micronaut
# Mirrors deploy-podman.sh from raspi-finance-endpoint (Spring Boot predecessor).
# Deploys to the same server/port/certs as a drop-in replacement.

REMOTE_HOST="debian-dockerserver"
REMOTE_USER="henninb"
REMOTE_DIR="/home/${REMOTE_USER}/raspi-finance-endpoint-micronaut"
HOST_IP="192.168.10.10"
ENV_FILE="env.prod"
APP="raspi-finance-endpoint-micronaut"
...
for container in "${CONTAINER_NAME}" raspi-finance-endpoint influxdb-server; do
...
[Unit]
Description=Raspi Finance Endpoint (Micronaut)
...
Volume=${REMOTE_DIR}/json_in:/opt/${APP}/json_in:rw
...
podman run -d \
  ...
  -v "${REMOTE_DIR}/json_in:/opt/${APP}/json_in:rw" \
...
if ssh "${REMOTE_HOST}" 'curl -k -f -s https://localhost:8443/health >/dev/null 2>&1'; then
...
if curl -k -f -s --connect-timeout 10 "https://${HOST_IP}:8443/health" >/dev/null 2>&1; then
...
log "  App diagnostics: ssh ${REMOTE_HOST} 'curl -k -s https://localhost:8443/health'"
